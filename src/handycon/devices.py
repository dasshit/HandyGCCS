...
def get_controller(handycon):
    ...
    # This will give us exclusive acces to the devices and their capabilities.
    for device in devices_original:
        if device.name == handycon.GAMEPAD_NAME \
            ...
            if handycon.CAPTURE_CONTROLLER:
                ...
                move(
                    handycon.controller_path,
                    str(HIDE_PATH / handycon.controller_event)
                )
            break
...
def get_keyboard(handycon):
    ...
    for device in devices_original:
        ...
        if device.name == handycon.KEYBOARD_NAME \
            ...
            if handycon.CAPTURE_KEYBOARD:
                ...
                move(
                    handycon.keyboard_path,
                    str(HIDE_PATH / handycon.keyboard_event)
                )
            break
...
def get_keyboard_2(handycon):
    ...
    for device in devices_original:
        handycon.logger.debug(f"{device.name}, {device.phys}")
        if device.name == handycon.KEYBOARD_2_NAME \
            ...
            if handycon.CAPTURE_KEYBOARD:
                ...
                move(
                    handycon.keyboard_2_path,
                    str(HIDE_PATH / handycon.keyboard_2_event)
                )
            break
        ...
        if handycon.keyboard_device:
            try:
                ...
                handycon.logger.error(
                    ...
                    f"{handycon.keyboard_device.name}"
                )
                handycon.logger.error(traceback.format_exc())
        ...
        if handycon.keyboard_2_device:
            try:
                async for seed_event_2 \
                        in handycon.keyboard_2_device.async_read_loop():
                    # Loop variables
        ...
        if handycon.controller_device:
            try:
                ...
                handycon.logger.error(
                    f"{err} | "
                    f"Error reading events from "
                    f"{handycon.controller_device.name}."
                )
                handycon.logger.error(traceback.format_exc())
